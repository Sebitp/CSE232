...
set <string> ReplacementCensor(istream & iss, ostream & oss, map<string,string> & m){
        ...
        while (isstream >> str){
            ...
            for (auto itr = m.begin(); itr != m.end(); ++itr){
                ...
                while (pos != std::string::npos){ //if it finds
                    ...
                    str.replace(pos,size(itr->first),itr->second); //i should replace from map value to original "str" var
...
int main() {
    string input_str ="note: this is a line with multiple WORds that should be rePLACEd bE./n all instances of word eveninlargerWordsshould be repLAced.other";
    istringstream input(input_str);
    ostringstream output;
    map<string, string> replacements = {{"word", "Grouped-Letter-Uni t"}, {"be", "wasp"}, {"not found", "not appearing"}, {"PlaCe", "LoCation"}, {"other", "j"}};
